...
int COpenCVAppGUIDlg::UpdateInspList()
{
	...
	_mInsps.insert(make_pair("OnInspHistogramColor", COpenCVAppGUIDlg::CallInspHistogramColor));
	_mInsps.insert(make_pair("OnGrayBrightness", COpenCVAppGUIDlg::CallGrayBrightness));
...
int COpenCVAppGUIDlg::CallInspHistogramColor(void* lpUserData)
{
	COpenCVAppGUIDlg* pDlg = reinterpret_cast<COpenCVAppGUIDlg*>(lpUserData);
	return pDlg->OnInspHistogramColor();
}
int COpenCVAppGUIDlg::CallGrayBrightness(void* lpUserData)
{
	COpenCVAppGUIDlg* pDlg = reinterpret_cast<COpenCVAppGUIDlg*>(lpUserData);
	return pDlg->OnGrayBrightness();
}
...
void COpenCVAppGUIDlg::OnBnClickedBtnInspectionCv()
{
	...
	auto f = _mInsps["OnGrayBrightness"];
	if (f == nullptr) return;
	auto ret = f(this); // int COpenCVAppGUIDlg::OnInspFindShapes()
...
int COpenCVAppGUIDlg::OnInspHistogram()
{
	...
	for (size_t row = 0; row < src_gray.rows; row++)
	{
		for (size_t col = 0; col <src_gray.cols; col++)
		{
			...
			ret.push_back((src_gray.data)[index]);
		}
	}
	int histo_max = 255;
	src_gray.copyTo(_mMatBuff[eImgDrawColor]);
	...
	for (int k = 0; k < histo.size(); k++ ) {
		double dx = (histo[k] / float(2800));
		int x = dx * 512;
		line(drawing, Point(0, k*2), Point(x, k*2), Scalar(0, 255, 0));
		//line(drawing, Point(0, k+1), Point(0, k+1), Scalar(0, 255, 0));
	}
	_bShowDebug = true;
	_bShowResult = true;
	Invalidate(FALSE);
...
void computeHistogram(vector<int>& histo, Mat& src_gray, int channel) {
	histo.resize(256, 0);
	for (size_t row = 0; row < src_gray.rows; row++) {
		for (size_t col = 0; col < src_gray.cols; col++) {
			int index = row * src_gray.cols + col;
			index *= 3;
			histo[(src_gray.data)[index + channel]]++;
		}
	}
}

void drawHistogram(vector<int>& histo_graph, Mat& drawing, Scalar color, int value) {
	int histo_max = 255;
	for (int k = 0; k < histo_graph.size(); k++) {
		double dx = (histo_graph[k] / float(value));
		int x = dx * drawing.rows;
		line(drawing, Point((k * 2) + 10, drawing.rows - x + 10), Point((k * 2) - 10, drawing.rows - x - 10), color, 4);
		line(drawing, Point((k * 2) - 10, drawing.rows - x + 10), Point((k * 2) + 10, drawing.rows - x - 10), color, 4);
	}
}

int COpenCVAppGUIDlg::OnInspHistogramColor()
{
	Mat src_gray = _mMatBuff[eImgSrcColor];
	vector<int> histoR, histoG, histoB;
	int cntR = 0, cntG = 0, cntB = 0;
	vector<int> histoR_graph, histoG_graph, histoB_graph;
	vector<int> value;
	computeHistogram(histoR, src_gray, 2);
	computeHistogram(histoG, src_gray, 1);
	computeHistogram(histoB, src_gray, 0);
	
	for (int i = 0; i < 256; i++) {
		histoR_graph.push_back(histoR[i]);
		histoG_graph.push_back(histoG[i]);
		histoB_graph.push_back(histoB[i]);
	}
	int max_value = 0;
	int elementR = *max_element(histoR_graph.begin(), histoR_graph.end());
	int elementG = *max_element(histoG_graph.begin(), histoG_graph.end());
	int elementB = *max_element(histoB_graph.begin(), histoB_graph.end());
	value.push_back(elementR);
	value.push_back(elementG);
	value.push_back(elementB);
	sort(value.begin(), value.end());
	max_value = value[value.size() - 1];
	Mat drawing = _mMatBuff[eImgDrawColor];

	drawHistogram(histoB_graph, drawing, Scalar(255, 0, 0), max_value);
	drawHistogram(histoG_graph, drawing, Scalar(0, 255, 0), max_value);
	drawHistogram(histoR_graph, drawing, Scalar(0, 0, 255), max_value);
...
int COpenCVAppGUIDlg::OnGrayBrightness()
{
	Mat src_gray = _mMatBuff[eImgSrcGray];
	src_gray.copyTo(_mMatBuff[eImgDrawColor]);
	src_gray.copyTo(_mMatBuff[eImgDebugColor]);
	Mat debug = _mMatBuff[eImgDebugColor];

	Mat brightness = _mMatBuff[eImgDrawColor];
	for (int row = 0; row < src_gray.rows; row++) {
		for (int col = 0; col < src_gray.cols; col++) {
			int index = row * src_gray.cols + col;
			int val = 30;
			if ((src_gray.data)[index]*1.24 + val < 256) {
				(brightness.data)[index] = (src_gray.data)[index]*1.24 +val;
			}else
			{
				(brightness.data)[index] = 255;
			}
		}
	}
	_bShowDebug = true;
