...
//행위 : 한다(직진, 자회전, 우회전, 정지...가속)
//객체간의 소통
//자동차를 운전하기 위해 명령을 보낸 상대 객체가 있어야 한다.
//운전자...운전자(주체).->.자동차(객체)...메시지 보냄

//클래스(class)
//유사한 특성(상태)와 행위를 갖는 개체를 표현하는 모형(틀)
//관념을 표현한다...추상화(abstraction)
//객체의 추상화가 ... class
//모든 객체는 클래스를 기반으로 생성 되며
//그 객체를 클래스의 인스턴스(instance)라고 합니다.
class Car	//class [클래스 이름]
{			//scope 시작
public:		//접근지정자 ... public/protectred/private
	Car();	//생성자...Creator...Ctor
	~Car(); //소멸자...Destroy...Dto
	void setMoney(int kw) { money = kw; }
	int getMoney() { return money; }

protected:
private:	//접근지정한정자 ... public / protectred / private
	int money;
};			//scope 끝
...
//추상화(Abstraction)	...일반화
// ...휴대폰..전화를 받고/걸고
// 정보은닉의 수단중 하나로 공통의 특성이나 기능을 묶어서 클래스로 정희한 것
//캡슐화(EnCapsulation)
